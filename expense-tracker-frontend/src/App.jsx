...
function App() {
  ...
  const addExpenseHandler = expense => {
    fetch(BASE_URL, {
      ...
      .then(res => res.json())
      .then(newExpense => setExpenses(prev => [...prev, { ...newExpense, isEditing: false }]))
  ...
  const editExpenseHandler = id => {
    setExpenses(prev => prev.map(exp => exp.id === id ? { ...exp, isEditing: true } : exp));
  };
  ...
  const saveExpenseHandler = (id, updatedExpense) => {
    fetch(`${BASE_URL}/${id}`, {
      ...
      .then(res => res.json())
      .then(data => setExpenses(prev => prev.map(exp => exp.id === id ? { ...data, isEditing: false } : exp)))
  ...
  return (
    <div>
      ...
      <Routes>
        ...
        />
        <Route path="/summary" element={<ExpenseSummary expenses={expenses} />} />
      </Routes>
    </div>
  );
}
